...
let
  ...
  # Shell-specific: echo "unset" if SANDBOX is not set, else its value
  checkSandboxVarCmd = {
    bash = "echo \${SANDBOX:-unset} > /tmp/sandbox-var";
    zsh = "echo \${SANDBOX:-unset} > /tmp/sandbox-var";
    fish = "set -q SANDBOX; and echo $SANDBOX > /tmp/sandbox-var; or echo unset > /tmp/sandbox-var";
  }.${shell};

  # Shell-specific: echo "unset" if SANDBOX_TEST is not set, else its value
  checkSandboxTestUnloadCmd = {
    bash = "echo \${SANDBOX_TEST:-unset} > /tmp/unload-check";
    zsh = "echo \${SANDBOX_TEST:-unset} > /tmp/unload-check";
    fish = "set -q SANDBOX_TEST; and echo $SANDBOX_TEST > /tmp/unload-check; or echo unset > /tmp/unload-check";
  }.${shell};
  ...
  test = testers.runNixOSTest {
    ...
    testScript = ''
      ...
      with subtest("direnv-sandbox off disables sandbox"):
          ...
          machine.send_chars("${checkSandboxVarCmd}\n")
      ...
      with subtest("direnv unloads when leaving disabled-sandbox dir"):
          ...
          machine.send_chars("${checkSandboxTestUnloadCmd}\n")
