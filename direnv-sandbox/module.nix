# NixOS module for programs.direnv.sandbox — direnv integration with sbox.
# Imports the sbox module so programs.sbox options are available.
{ wrappers, sboxModule }:
{
  lib,
  config,
...
in
{
  ...
  config = lib.mkIf (direnv-cfg.enable && cfg.enable) {
    ...
    # Add sandbox-aware hook sourcing.
